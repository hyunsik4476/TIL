...
* 시퀀스형 컨테이너에 패킹/ 언패킹 연산자 * 를 사용하여 가능

#### 패킹

* 대입문의 좌변 변수에 *

* 초과하는 우변 객체가 * 표시된 변수에 리스트로 대입
...
* 리스트 등을 튜플 형태로 함수에 대입 가능
* argument unpacking: `*argument`
...
* `+`, `-`, `*`, `/`, `//`, `%` `**`
* 나눗셈은 항상 결과가 float임
* `divmod(x, y)`로 (몫, 나머지) 형태의 튜플로 받을 수 있음



### 비교 연산자

* 값을 비교해 True/ False 값을 리턴
* `>=`, `<=` 순서 틀리지 말 것
